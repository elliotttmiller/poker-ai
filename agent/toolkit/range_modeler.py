...
class RangeModeler:
    ...
    def get_position_range(
        self, position: str, action: str = "open", stack_depth: str = "100bb"
    ) -> HandRange:
        """
    ...
    def estimate_opponent_range(
        ...
        adjusted_range = self._adjust_range_for_player_type(baseline_range, opponent_stats)

        # Further adjust based on action sequence
        final_range = self._adjust_range_for_actions(adjusted_range, action_sequence, board)

        return final_range

    def _adjust_range_for_player_type(
        self, baseline_range: HandRange, stats: Dict[str, Any]
    ) -> HandRange:
    ...
    def _adjust_range_for_actions(
        self, range_estimate: HandRange, action_sequence: List[str], board: List[str] = None
    ) -> HandRange:
    ...
    def calculate_range_advantage(
        self, our_range: HandRange, opponent_range: HandRange, board: List[str]
    ) -> Dict[str, Any]:
        """
