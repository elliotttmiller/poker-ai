...
class PostGameAnalyzer:
    ...
    def _analyze_session_overview(self, session_logs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        actions_taken = [log.get("final_action", {}).get("action", "unknown") for log in session_logs]
    ...
    def _analyze_positional_play(self, session_logs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        positional_stats = defaultdict(lambda: {
            "hands_played": 0,
    ...
    def _analyze_multi_way_performance(self, session_logs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            "pot_type_distribution": multi_way_stats,
            "identified_leaks": leaks
        }
        
    def _analyze_board_texture_performance(self, session_logs: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Analyze performance on different board textures."""
        texture_stats = defaultdict(lambda: {
            "hands_played": 0,
            "actions": defaultdict(int),
            "confidence_scores": []
        })
    ...
    def _analyze_betting_patterns(self, session_logs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if betting_stats["bet_sizes"]:
            ...
            bet_size_variance = statistics.stdev(betting_stats["bet_sizes"]) if len(betting_stats["bet_sizes"]) > 1 else 0
    ...
    def _analyze_opponent_adaptation(self, session_logs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        opponent_interactions = defaultdict(lambda: {
            "encounters": 0,
        ...
        for log in session_logs:
            ...
            for opponent_name, opponent_data in opponents.items():
                ...
                if "stats" in opponent_data:
                    opponent_interactions[opponent_name]["opponent_stats_observed"] = opponent_data["stats"]
    ...
    def _calculate_statistical_deviations(self, session_logs: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def generate_improvement_report(
        ...
        if overview:
            ...
            report_lines.append(f"Average Decision Confidence: {overview.get('average_confidence', 0.0):.1%}")
        ...
        if priority_improvements:
            ...
            for i, improvement in enumerate(priority_improvements, 1):
                report_lines.append(f"{i}. [{improvement['category'].upper()}] {improvement['description']}")
        ...
        if priority_improvements:
            ...
            for improvement in priority_improvements[:3]:  # Top 3
                ...
                if category == "positional":
                    ...
                    report_lines.append("  - Study continuation betting frequencies on different textures")
                    report_lines.append("  - Improve range advantage recognition")
                    
                report_lines.append("")
        else:
            report_lines.append("No major leaks identified. Continue current strategy and focus on:")
