...
def calculate_pot_equity_needed(
    pot_size: float, bet_to_call: float, implied_odds_multiplier: float = 1.0
) -> float:
    """
...
def analyze_blockers(
    our_cards: List[str], board_cards: List[str], target_hands: List[str]
) -> Dict[str, Any]:
    """
    ...
    overall_effect = (
        sum(blocking_effects.values()) / len(blocking_effects) if blocking_effects else 0.0
    )
...
def _calculate_single_hand_blocking(blocked_cards: set, target_hand: str) -> float:
    ...
    blocked_target_cards = len(
        [card for card in target_cards if any(card.startswith(c[0]) for c in blocked_cards)]
    )
...
def calculate_continuation_bet_frequency(
    position: str, board_texture: str, opponent_type: str = "balanced"
) -> float:
    """
    ...
    final_frequency = (
        base_frequency + position_adjustment + texture_adjustment + opponent_adjustment
    )
...
def _get_implied_odds_recommendation(
    is_profitable: bool, expected_value: float, stack_risk_ratio: float, win_probability: float
) -> str:
...
def calculate_reverse_implied_odds(
    ...
    return {
        ...
        "recommendation": (
            "Consider folding strong draws on dangerous boards"
            if reverse_implied_factor > 0.4
            else "Reverse implied odds manageable"
        ),
    }
