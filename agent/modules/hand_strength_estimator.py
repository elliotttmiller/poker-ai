...
class HandStrengthEstimator:
    ...
    def estimate(self, game_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            pot_size = game_state.get("pot_size", 100)  # Get pot size for dynamic adjustment

            if self.use_fallback:
                return self._fallback_estimation(hole_cards, community_cards, street, pot_size)
            else:
                return self._onnx_estimation(hole_cards, community_cards, street, pot_size)

        except Exception as e:
            self.logger.error(f"Hand strength estimation error: {e}")
            return self._get_default_estimate()

    def _onnx_estimation(
    ...
    def _fallback_estimation(
        ...
        if street == "preflop":
            return self._estimate_preflop_strength(hole_cards, pot_size)
        else:
