...
class LearningModule:
    ...
    def __init__(self, output_directory: str = "data/hand_history/"):
        ...
        self.hand_history_file = os.path.join(
            ...
            f"hand_history_{session_id}.jsonl"
        )
    ...
    def _async_log_hand(
        ...
        try:
            ...
            if hasattr(decision_packet, '__dict__'):
                packet_data = asdict(decision_packet) if hasattr(decision_packet, '__dataclass_fields__') else vars(decision_packet)
            else:
            ...
            self.logger.debug(f"Hand logged successfully - total hands: {self.session_stats['hands_logged']}")
    ...
    def _create_learning_record(
        ...
        learning_record = {
            ...
            'decision_features': {
                ...
                'required_equity': packet_data.get('synthesizer_analysis', {}).get('required_equity', 0),
            ...
            # Performance metrics
            'performance_metrics': {
                'decision_quality_score': self._calculate_decision_quality_score(packet_data, hand_outcome),
    ...
    def export_training_data(
        ...
        if not output_file:
            ...
            output_file = os.path.join(
                self.output_directory, 
                f"training_export_{timestamp}.jsonl"
            )
    ...
    def _passes_filter(self, record: Dict[str, Any], filter_criteria: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if 'min_confidence' in filter_criteria:
                if record.get('decision_features', {}).get('confidence_score', 0) < filter_criteria['min_confidence']:
                    return False
            ...
            if 'min_quality_score' in filter_criteria:
                if record.get('performance_metrics', {}).get('decision_quality_score', 0) < filter_criteria['min_quality_score']:
                    return False
