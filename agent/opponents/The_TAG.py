...
class TightAggressivePlayer:
    ...
    def __init__(self, name: str = "TAG_Player"):
        ...
        self.hand_ranges = {
            ...
            "late": HandRange(
                ...
                playable=["66", "55", "44", "33", "22", "A9s", "A8s", "A7s", "KTs", "K9s", 
    ...
    def _get_bet_sizing(self, action_type: str, pot_size: int, our_stack: int, 
    ...
    def declare_action(self, valid_actions: List[Dict[str, Any]], hole_cards: List[str], 
        ...
        try:
            ...
            self.logger.debug(f"TAG decision: {street} street, position: {position}, "
                            f"cards: {hole_cards}, pot: {pot_size}")
            
            # Preflop decision logic
            if street == "preflop":
                return self._decide_preflop(hole_cards, position, valid_actions, 
                                          pot_size, our_stack, round_state)
            else:
                return self._decide_postflop(hole_cards, round_state.get("community_card", []),
    ...
    def _decide_preflop(self, hole_cards: List[str], position: str, 
        ...
        has_raise = any("raise" in str(action) for action in round_state.get("action_histories", {}).get("preflop", []))
    ...
    def _decide_postflop(self, hole_cards: List[str], community_cards: List[str],
                        valid_actions: List[Dict[str, Any]], pot_size: int, 
                        our_stack: int, round_state: Dict[str, Any]) -> Tuple[str, int]:
