...
class LooseAggressivePlayer:
    ...
    def __init__(self, name: str = "LAG_Player"):
        ...
        self.hand_ranges = {
            "early": LAGHandRange(
                ...
                playable=["77", "66", "55", "44", "33", "22", "A9s", "A8s", "A7s", "A6s", "A5s", "A4s", "A3s", "A2s",
                         "KTs", "K9s", "K8s", "QTs", "Q9s", "JTs", "J9s", "T9s", "98s", "87s", "76s", "65s"],
                bluff=["K9", "K8", "K7", "Q9", "Q8", "J9", "J8", "T8", "97", "86", "75", "64"]
            ),
            "middle": LAGHandRange(
                premium=["AA", "KK", "QQ", "JJ", "TT", "AKs", "AK", "AQs", "AQ"],
                strong=["99", "88", "77", "AJs", "AJ", "KQs", "KQ", "KJs", "KJ", "QJs", "QJ", "JTs"],
                playable=["66", "55", "44", "33", "22", "A9s", "A8s", "A7s", "A6s", "A5s", "A4s", "A3s", "A2s",
                         "KTs", "K9s", "K8s", "K7s", "QTs", "Q9s", "Q8s", "JTs", "J9s", "J8s", "T9s", "T8s", 
                         "98s", "87s", "76s", "65s", "54s"],
                bluff=["K9", "K8", "K7", "K6", "Q9", "Q8", "Q7", "J9", "J8", "J7", "T8", "T7", "97", "96", "86", "85", "75", "74", "64", "63"]
            ),
            "late": LAGHandRange(
                premium=["AA", "KK", "QQ", "JJ", "TT", "99", "AKs", "AK", "AQs", "AQ"],
                strong=["88", "77", "66", "55", "AJs", "AJ", "A9s", "A9", "KQs", "KQ", "KJs", "KJ", "KTs", "QJs", "QJ", "JTs"],
                playable=["44", "33", "22", "A8s", "A7s", "A6s", "A5s", "A4s", "A3s", "A2s", "K9s", "K8s", "K7s", "K6s", "K5s",
                         "QTs", "Q9s", "Q8s", "Q7s", "JTs", "J9s", "J8s", "J7s", "T9s", "T8s", "T7s", "98s", "97s", "87s", "86s", "76s", "75s", "65s", "64s", "54s", "53s", "43s"],
    ...
    def _evaluate_hand_for_lag_play(self, hole_cards: List[str], position: str) -> Tuple[str, float]:
        """
    ...
    def _get_lag_bet_sizing(self, action_type: str, pot_size: int, our_stack: int,
                           valid_actions: List[Dict[str, Any]], aggression_score: float) -> int:
    ...
    def declare_action(self, valid_actions: List[Dict[str, Any]], hole_cards: List[str], 
        ...
        try:
            ...
            self.logger.debug(f"LAG decision: {street} street, position: {position}, "
                            f"cards: {hole_cards}, pot: {pot_size}")
            
            # Preflop decision logic
            if street == "preflop":
                return self._decide_preflop_lag(hole_cards, position, valid_actions, 
                                              pot_size, our_stack, round_state)
            else:
                return self._decide_postflop_lag(hole_cards, round_state.get("community_card", []),
    ...
    def _decide_preflop_lag(self, hole_cards: List[str], position: str, 
        ...
        if not has_raise:
            ...
            if category in ["premium", "strong", "playable"]:
                self.pfr_hands += 1
                self.total_bets += 1
                amount = self._get_lag_bet_sizing("raise", pot_size, our_stack, valid_actions, aggression_score)
                return "raise", amount
            else:  # Bluff hands
                ...
                if position == "late" or random.random() < 0.6:
                    ...
                    self.total_calls += 1
                    amount = self._get_lag_bet_sizing("call", pot_size, our_stack, valid_actions, aggression_score)
                    return "call", amount
        else:
    ...
    def _decide_postflop_lag(self, hole_cards: List[str], community_cards: List[str],
                            valid_actions: List[Dict[str, Any]], pot_size: int, 
                            our_stack: int, round_state: Dict[str, Any]) -> Tuple[str, int]:
        ...
        try:
            ...
            if self.last_action_was_raise and not facing_bet:
                ...
                if random.random() < c_bet_frequency or hand_strength > 0.5:
                    ...
                    self.total_bets += 1
                    amount = self._get_lag_bet_sizing("raise", pot_size, our_stack, valid_actions, 0.7)
                    return "raise", amount
            ...
            if hand_strength > 0.7:
                if facing_bet:
                    ...
                    if random.random() < 0.7:  # 70% raise frequency with strong hands
                        self.value_bets += 1
                        self.total_bets += 1
                        amount = self._get_lag_bet_sizing("raise", pot_size, our_stack, valid_actions, 0.8)
                        return "raise", amount
                    else:
                        self.total_calls += 1
                        amount = self._get_lag_bet_sizing("call", pot_size, our_stack, valid_actions, 0.7)
                        return "call", amount
                else:
                    # Bet for value
                    self.value_bets += 1
            ...
            # Medium strength hands with draws
                ...
                if facing_bet:
                    ...
                    if draw_potential > 0.7 and random.random() < 0.5:
                        self.total_bets += 1
                        amount = self._get_lag_bet_sizing("raise", pot_size, our_stack, valid_actions, 0.6)
                        return "raise", amount
                    elif hand_strength > 0.4 or draw_potential > 0.5:
                        self.total_calls += 1
                        amount = self._get_lag_bet_sizing("call", pot_size, our_stack, valid_actions, 0.5)
                        return "call", amount
                    else:
                        return "fold", 0
                else:
                    ...
                    if hand_strength > 0.5 or (draw_potential > 0.6 and random.random() < 0.6):
                        ...
                        # Check
                        amount = self._get_lag_bet_sizing("call", pot_size, our_stack, valid_actions, 0.3)
                        return "call", amount
            ...
            # Weak hands - bluff or fold
                ...
                if facing_bet:
                    ...
                    if random.random() < 0.2:  # 20% bluff frequency
                        self.bluffs_attempted += 1
                        self.total_bets += 1
                        amount = self._get_lag_bet_sizing("raise", pot_size, our_stack, valid_actions, 0.4)
                        return "raise", amount
                    else:
                        return "fold", 0
                else:
                    ...
                    if random.random() < 0.4:  # 40% bluff frequency when checked to
                        ...
                        # Check
                        amount = self._get_lag_bet_sizing("call", pot_size, our_stack, valid_actions, 0.2)
                        return "call", amount
