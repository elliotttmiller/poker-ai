...
def test_player_elimination():
    ...
    assert (
        tournament.eliminated_players[0].finishing_place == 4
    )  # 4th place (4 players remaining when eliminated)
...
def test_tournament_completion():
    ...
    tournament = TournamentDirector(
        "test_completion", initial_stack=500
    )  # Smaller stacks for faster completion
...
def test_side_pot_edge_case():
    ...
    assert abs(initial_total_chips - final_total_chips) <= len(
        tournament.players
