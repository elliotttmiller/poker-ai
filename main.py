            ...
            results.append(
                {
                    ...
                    "action": action,
                    "confidence": (
                        decision_packet.confidence
                        if hasattr(decision_packet, "confidence")
                        else 0.7
                    ),
                    "reasoning": (
                        decision_packet.reasoning_summary
                        if hasattr(decision_packet, "reasoning_summary")
...
def create_game_config(args):
    ...
    try:
        ...
        if hasattr(pokermind_agent, "cognitive_core") and hasattr(
            pokermind_agent.cognitive_core, "synthesizer"
        ):
...
def run_simulation(args):
    ...
    logger.info(
        f"Configuration: {args.max_rounds} rounds, {args.initial_stack} initial stack, {args.agent_style} style"
    )
...
Examples:
  ...
  %(prog)s --mode profile --max_rounds 100   # Performance profiling
    ...
    parser.add_argument(
        "--no-log", action="store_true", help="Disable detailed logging for faster runs"
    )
    ...
    parser.add_argument(
        "--version", action="version", version="PokerMind v1.0 (Phase 5 Final)"
    )
